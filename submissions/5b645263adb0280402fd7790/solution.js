...
module.exports = class {
    ...
    recalEnemyCosts(o) {
        ...
        if (this.currentRound > 1) {
            ...
            offerToEnemy.forEach((items, index) => {
                if (items > 0) {
                    const lost = this.enemyValues[index] * (0.08 * items);
        ...
        o.forEach((items, itemIndex) => {
            ...
            } else if (this.enemyValues[itemIndex] < 0.38 && items === this.counts[itemIndex]) {
    ...
    offer(o) {
        ...
        if (o) {
            ...
            if (this.enemyOffersLeft() === 0) {absoluteAcceptCost = (3 - (this.me * 2))}

            const relativeAcceptCost = costForEnemy *
                (1.3 - 0.13 * (this.currentRound));
        ...
        for (let i = 0; i < this.orderMap.length && !offer; i++) {
            ...
            const canRepeat = itemsInHands <= (this.myOffersLeft() - 1 + this.me);
            ...
            this.log(`(${!alreadyGivvenOffer} || ${canRepeat}) && ${offerHaveCostForEnemy} ${currentRelativeCostMoreThanLastOffer} ${offerCostForMeMoreThanMinLevel}`);
        ...
        const wantDoSameOffer = (offerCost >= offer.cost);
...
// Получаем стоимость офера по указанным ценам
